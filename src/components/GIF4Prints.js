...
import FourStrips from "../images/makePayment4strips.png";
  ...
  useEffect(() => {
    if (isInitialMount.current) {
      ...
      sseSource.onmessage = function logEvents(event) {
        ...
        sseSource.close();
        history.push("/paymentsuccessgif4");
      };
    }
  });
        ...
        <Grid container item xs={6} className="makePaymentLeft">
          <Grid item xs={12} container={true}>
            <div className="makePaymentLeftDiv">
              <h1 className="makePaymentLeftTitle">4 Bookmark Printouts</h1>
              <span>with softcopies emailing</span>
              <img
                src={FourStrips}
                className="makePaymentLeft4BookmarkPrintoutsImage"
              />
            </div>
          </Grid>
        </Grid>
